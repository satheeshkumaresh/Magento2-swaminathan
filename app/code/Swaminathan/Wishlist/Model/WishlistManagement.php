...
use Swaminathan\Wishlist\Api\WishlistManagementInterface;
...
class WishlistManagement implements WishlistManagementInterface
{
    ...
    {
        $collectiondata= $this->productCollection->addAttributeToSelect('*')->addFieldToFilter('entity_id',$productId);
        $productCount=$collectiondata->getData();
         if ($productId == null || count($productCount)==0) {      
            $response[] =[
                "code" => 400,
                "status" => false,
            ...
            $wishlistItemCollection = $this->_wishlistCollectionFactory->create()
            ->addCustomerIdFilter($customerId)->addFieldToFilter('product_id',$productId);
            $productCount=count($wishlistItemCollection->getData());
               if($productCount > 0){
                $response[] =[
                    "code" => 400,
                    "status" => false,
                    "message" => "This product already added in wishlist"
                ];
               }else{
                ...
                ($customerId, true);
                $wishlist->addNewItem($product,['qty' => $minQty]);
                $wishlist->save();
                $response[] =[
                    "code" => 200,
                    "status" => true,
